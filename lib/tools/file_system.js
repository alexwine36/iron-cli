 ...
 */
module.exports.injectIntoFile = function injectIntoFile(filepath, content) {
    if (!this.isFile(filepath)) {
      this.logError("No file found to inject content into at path " + JSON.stringify(filepath));
      return false;
    }

    var methodText = "Meteor.methods({"
    var raw = fs.readFileSync(filepath, 'utf8');
    var addedContent = methodText + "\n" + content + "\n";
    var newFileContent = raw.replace(methodText, addedContent);
